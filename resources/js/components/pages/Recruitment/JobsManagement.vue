  ...
  <div>
    ...
    <div class="container bg-base-100 p-3">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-4">
        <CardJobM v-for="(item, index) in filteredSuggestions" :key="item.id" :title="item.title" :salary="item.salary"
          :id="item.id" :link="`/Detail-Job/${item.id}`" @delete-item="deleteJob(item.id)" :negotiable="item.negotiable"
          :imgUrl="user.avatar_url" :status="item.status" :date="item.deadline" />
        <div @click="addJob"
